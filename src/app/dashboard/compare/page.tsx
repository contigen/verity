  ...
  return (
    <div className="container py-8">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">Analyze similarity between multiple wallets to detect Sybil clusters</p>
      </div>

      <Card className="border-border mb-8">
        <CardHeader>
          <CardTitle className="text-xl">Add Wallets to Compare</CardTitle>
          <CardDescription>Enter 2-5 wallet addresses to analyze their relationships</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col gap-4">
            <div className="flex gap-2">
              ...
              />
              <Button onClick={addWallet} disabled={!inputValue || wallets.length >= 5 || isComparing}>
                Add
              </Button>
            </div>
            ...
            {wallets.length > 0 && (
              <div className="space-y-2">
                {wallets.map((wallet) => (
                  <div key={wallet} className="flex items-center justify-between p-2 bg-secondary rounded-md">
                    <span className="font-mono text-sm truncate">{wallet}</span>
                    <Button variant="ghost" size="icon" onClick={() => removeWallet(wallet)} disabled={isComparing}>
                      <X className="h-4 w-4" />
                    </Button>
                  </div>
                ))}
            ...
            <div className="flex justify-end mt-2">
              <ButtonGold onClick={handleCompare} disabled={wallets.length < 2 || isComparing}>
                {isComparing ? (
                  <>
        ...
        <>
          <Card className="border-border mb-8">
            <CardHeader>
              ...
              <CardDescription>Analysis of wallet similarities and potential Sybil connections</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {comparisonResults.map((result) => (
                    <TableRow key={result.wallet}>
                      <TableCell className="font-mono text-xs">{result.wallet}</TableCell>
          ...
          <Card className="border-border">
            <CardHeader>
              ...
              <CardDescription>Visual representation of wallet relationships</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="h-[400px] relative network-animation rounded-md border border-border p-4">
                ...
                <svg className="absolute inset-0 w-full h-full" xmlns="http://www.w3.org/2000/svg">
                  <line x1="50%" y1="50%" x2="30%" y2="30%" stroke="hsl(var(--border))" strokeWidth="1" />
                  <line
                    x1="50%"
                    y1="50%"
                    x2="70%"
